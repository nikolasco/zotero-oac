...
var Zotero_Browser = new function() {
	...
	var tools = {
		...
		'zotero-annotate-image-tb-rectangle': {
			cursor: "text",
			event: "mouseup",
			callback: function(e){
		...
		'zotero-annotate-image-tb-polygon': {
			cursor: "text",
			event: "mouseup",
			callback: function(e){
				alert("POLY");
	...
	function contentLoad(event) {
		...
		var MIMEType = Zotero.Items.get(annotationID).attachmentMIMEType;
			...
			if (annotationID) {
				if (MIMEType.substring(0, 5) != "image") {
					...
					if (!tab.page.axeImg) {
						tab.page.axeImg = new Zotero.AXEImage(this, browser, annotationID);
						tab.page.axeImg.loadImageFromPage();
					}
				}
	...
	function updateStatus() {
		...
		// itemType 14 is artwork
		
	
		// set annotation bar status
		if(tab.page.annotations) {
		
				var itemID = tab.page.annotations.itemID;
		        var MIMEType = Zotero.Items.get(itemID).attachmentMIMEType;
				document.getElementById('zotero-annotate-image-tb').hidden=true;
				document.getElementById('zotero-annotate-tb').hidden = false;
				toggleMode();
			
		
		} 
		else if (tab.page.axeImg){
		

				document.getElementById('zotero-annotate-image-tb').hidden=false;
	...
	function zoomIn(){
		var tab = _getTabObject(Zotero_Browser.tabbrowser.selectedBrowser);
		tab.page.axeImg.zoomIn();
	
	}
	function zoomOut(){
		var tab = _getTabObject(Zotero_Browser.tabbrowser.selectedBrowser);
		tab.page.axeImg.zoomOut();
	}
	function _AXE(type,e){
		var tab = _getTabObject(Zotero_Browser.tabbrowser.selectedBrowser);
		var axe = tab.page.axeImg;
